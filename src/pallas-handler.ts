...
type OriginalRequest = AuthRequest
type ProviderData = Record<string, any>
...
app.get("/authorize", async (c) => {
    ...
    const backendUrl = new URL("/auth/authorize", "http://localhost:8080");
...
const fetchAuthenticationResult = async (code: string, state: string): Promise<CallbackResponse> => {
    const backendUrl = new URL("/auth/callback", "http://localhost:8080");
...
app.get("/callback", async (c) => {
    ...
    try {
        ...
        const oauthReqInfo = authResult.req_info;
        ...
        const scope = authResult?.provider_data?.permissions

        const { redirectTo } = await c.env.OAUTH_PROVIDER.completeAuthorization({
            request: oauthReqInfo,
            userId: authResult.provider_data.user_id,
            metadata: {},
            scope,
            props: {
                accessToken: authResult.access_token,
                organizationId: authResult.provider_data.organization_id,
                permissions: scope,
                refreshToken: authResult.refresh_token,
                user: authResult.provider_data.user,
            }
        });
