...
app.get("/callback", async (c) => {
    try {
        ...
        const url = `http://localhost:8080/auth/sso/callback?${params.toString()}`
        console.info(url)
        console.info("me url")
        ...
        const { access_token, refresh_token, user, permissions = [], organization_id, oauth_req_info: oauthReqInfo } = json;
        ...
        const { codeChallenge, codeChallengeMethod, ...cleanOauthReqInfo } = oauthReqInfo;
        ...
        const { redirectTo } = await c.env.OAUTH_PROVIDER.completeAuthorization({
            request: {
                clientId: cleanOauthReqInfo.clientId, //c.req.query("client_id") || "",
                redirectUri: cleanOauthReqInfo.redirectUri,  //c.req.query("redirect_uri") || "",
                responseType: "code",
...
app.post("/token", async (c) => {
    console.info('TOKEN ENDPOINT HIT!');  // Add this
    console.info('Request headers:', Object.fromEntries(c.req.raw.headers.entries()));
    console.info('Request body preview:', await c.req.raw.clone().text());

    const fastApiUrl = "http://localhost:8080";

    // Forward the entire request to FastAPI
    const response = await fetch(`${fastApiUrl}/auth/sso/token`, {
        method: "POST",
        headers: {
